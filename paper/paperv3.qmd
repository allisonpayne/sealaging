...
pheno_model <- seal_phenology %>% 
  ...
  pivot_longer(moltdur:breeddur, 
               names_to = "phase", 
               values_to = "dur_days") %>% 
  glmer.nb(dur_days ~ age10:phase:age_cat + (1 | year_fct), 
           data = .)
...
breed_duration_plot <- ggpredict(
  ...
  geom_ribbon(aes(ymin = dur_days_lwr, ymax = dur_days_upr, fill = age_cat),
              alpha = 0.25) +
  geom_line(aes(y = dur_days, color = age_cat, linetype = age_cat)) +
  # Summarized data
...
extract_p <- \(h) map_dbl(mod_sens, \(m) pnorm(m[[h]]["age10:age_catOld", "z value"]))
...
ggplot(bioage_pred_wgt, aes(bioage, predicted)) +
  ...
  scale_color_manual(values = c("#7fbc41", "#de77ae")) +
  scale_fill_manual(values = c("#7fbc41", "#de77ae")) +
  labs(x = "Biological age (years before death)") +
  theme(legend.position = "none") + 
  geom_text(aes(label = n), y = 1, vjust = -0.5)
