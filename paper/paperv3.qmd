...
pup_survival_model <- glmer(pup_survived ~ poly(age, 2) + (1 | animalID) + (1 | year),
                            ...
                            family = "binomial")
summary(pup_survival_model)
#significant result, plot as quadratic on figure

recruit_dat <- sealdat %>% mutate(age10 = age / 10)

pup_recruit_model <- glmer(puprecruited ~ poly(age, 2) + (1 | animalID) + (1 | year),
                           data = filter(recruit_dat, year > 1984 & year < 2019), 
                           family = "binomial", 
                           control = glmerControl(optimizer = "bobyqa"))
summary(pup_recruit_model)
#divide age by 10 (ie rescale variables) - did not work
#increase max iterations within glmer? 
#change the optimizer? -> worked  

#survival but not recruitment success with age
#figure with both models, with the recruitment one dashed or something

#ranef variance goes in the table legend 
