...
# fixed + random effect predictions
year_ranef <- ranef(breed_age_mod)$year_fct %>% 
  as_tibble(rownames = "year_fct") %>% 
  rename(year_ranef = `(Intercept)`)
  
breed_pred <- breed_grid %>% 
  mutate(breed_fixef = predict(breed_age_mod, 
                               newdata = breed_grid,
                               re.form = NA)) %>% 
  left_join(year_ranef, by = "year_fct") %>% 
  mutate(mean_breed = breed_fixef + year_ranef)

# add confidence intervals
mm <- model.matrix(~ age_cat:age10, breed_grid)
pvar1 <- diag(mm %*% tcrossprod(vcov(breed_age_mod), mm))
breed_pred$ci_low <- breed_pred$mean_breed - 1.96 * sqrt(pvar1)
breed_pred$ci_high <- breed_pred$mean_breed + 1.96 * sqrt(pvar1)

# Apply weighted average
seal_years <- count(sealdat, year_fct)
breed_pred_wgt <- breed_pred %>% 
  left_join(seal_years, by = "year_fct") %>% 
  group_by(age, age_cat) %>% 
  summarize(across(c(mean_breed, ci_low, ci_high), \(x) weighted.mean(x, n)),
            .groups = "drop") %>% 
  # invert logit
  mutate(across(c(mean_breed, ci_low, ci_high), 
                family(breed_age_mod)$linkinv))
  
ggplot(breed_pred_wgt, aes(age)) +
  geom_line(aes(y = predicted, group = interaction(age_cat, year_fct)),
            breed_pred_years,
            alpha = 0.1) +
  geom_ribbon(aes(fill = age_cat, ymin = ci_low, ymax = ci_high), 
              alpha = 0.5) +
  geom_line(aes(y = mean_breed, color = age_cat),
            linewidth = 1.2) +
...
fig_recr <- ggplot(recr_pred_pop, aes(age, predicted)) +
  ...
  geom_ribbon(aes(fill = age_cat, ymin = conf.low, ymax = conf.high), 
              alpha = 0.2) +
  geom_line(aes(color = age_cat), linewidth = 1.2, linetype = "dashed") +
