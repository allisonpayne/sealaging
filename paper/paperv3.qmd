...
Sample size n = `r nrow(surv_recr_data)` observations of `r n_distinct(surv_recr_data$animalID)` individuals.
...
longevity_est <- summary(breed_sel_dis)$coefficients["longevity", "Estimate"]
longevity_se <- summary(breed_sel_dis)$coefficients["longevity", "Std. Error"]
longevity_coef <- sprintf(
  "%0.5f (95%% CI: %0.5f - %0.5f)", 
  longevity_est,
  longevity_est - 1.96 * longevity_se,
  longevity_est + 1.96 * longevity_se
)
...
Selective disappearance did not explain the observed pattern of declining breeding probabilities with age. The coefficient for longevity was estimated at `r longevity_coef`, which was not significantly different than 0 ($p$ = `r longevity_p`). The predicted breeding probabilities for seals with longevity of 5, 10, and 17 years are shown in @fig-sel-dis-h1.
