...
fit_models <- function(a) {
  ...
  effect_size <- function(m) {
    probs <- tibble(age = c(15, 19),
                    age_cat = c("Old", "Old")) %>% 
      mutate(age10 = (age - a) / 10) %>% 
      predict(m, re.form = NA, newdata = ., type = "response")
    odds <- probs / (1 - probs)
    odds[2] / odds[1]
  }
  ...
  list(
    ...
    h1_effect = effect_size(breed_age_mod),
    h2_surv_effect = effect_size(pup_surv_mod),
    h2_recr_effect = effect_size(pup_recr_mod),
    h3_effect = effect_size(sexrat_age_mod)
  )
}
...
p_tbl <- tibble(age_cutoff = cutoff, 
                "H1 (breeding)" = extract_p("h1", tails = 2),
                "H2a (survival)" = extract_p("h2_surv", tails = 2),
                "H2b (recruitment)" = extract_p("h2_recr", tails = 2),
                "H3 (sex ratio)" = extract_p("h3", tails = 2))
...
ggplot(detection_pred, aes(age)) +
  # Line per year
  geom_line(aes(y = predicted, group = interaction(age, year_fct)), 
            detection_pred_years, 
            alpha = 0.2) +
  # Weighted model
...
| 2     | 3        | 4        | 5        | 6        | 7        | 8        | 9        |
+=======+==========+==========+==========+==========+==========+==========+==========+
| 22±0  | 23.7±1.6 | 26.6±1.6 | 26.1±1.9 | 26.5±2.2 | 27.1±1.2 | 28.2±0.8 | 28.5±0.7 |
|       |          |          |          |          |          |          |          |
| \(1\) | \(9\)    | \(24\)   | \(12\)   | \(6\)    | \(8\)    | \(5\)    | \(2\)    |
+-------+----------+----------+----------+----------+----------+----------+----------+
