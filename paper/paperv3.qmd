...
breed_pred_years <- ggpredict(
  ...
  filter((age_cat == "Young" & age < age_senesce) |
           (age_cat == "Old" & age >= age_senesce),
         age <= 20)
breed_pred_wgt <- breed_pred_years %>% 
...
ggplot(breed_pred_pop, aes(age, predicted)) +
  ...
  scale_y_continuous("Breeding probability", labels = scales::percent) +
...
ggsave("outputs/repro_senesce.svg", width = 11, height = 6, units = "in")
```
...
annual_plot <- ggplot(timing_summary, aes(x = age, y = doy, fill = event)) + 
  ...
  # geom_text(aes(label = n, color = "white", y = 300))
...
(annual_plot | breed_duration_plot | molt_duration_plot) + plot_annotation(tag_levels = "A") + plot_layout(ncol = 1, nrow = 3)
```
