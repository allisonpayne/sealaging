...
sealdat <- read_csv(here::here("data/raw/128L pull 2023_12_05.csv"), 
  ...
  group_by(animalID) %>% 
  # FIXME
  mutate(firstrepro = min(age),
         lastrepro = ifelse(max(year) < 2020,
                            max(age),
                            NA),
         lastrepro10 = lastrepro / 10,
...
do.call(AIC, h1_selective_models) %>% 
  as_tibble() %>% 
  mutate(model = c("Base", 
                   "Selective appearance", 
                   "Selective disappearance", 
...
selapp_pred <- ggpredict(
  ...
  terms = c("age10 [all]", "age_cat", "firstrepro [4,6]")
) %>% 
  as_tibble() %>% 
  mutate(age = x * 10 + age_senesce,
         age_cat = factor(group, levels = c("Young", "Old")),
...
Selective appearance and disappearance did not explain the observed pattern of declining breeding probabilities with age. The AIC weights for the base, selective appearance, selective disappearance, and combined models were 0.526, 0.197, 0.201, and 0.076, respectively. These weights indicate the most likely explanation for the observed pattern is within-individual change, rather than selective appearance or disappearance. In the selective appearance and disappearance models, the coefficients for age at first reproduction (95% CI: -0.091 - 0.112) and last reproduction (95% CI: -0.037 - 0.051) were not significantly different than 0, respectively. Furthermore, predictions of the selective appearance and disappearance models did not meaningfully differ from the base model @fig-selective-h1.
...
do.call(AIC, h2_selective_models) %>% 
  as_tibble() %>% 
  mutate(model = c("Base", 
                   "Selective appearance", 
                   "Selective disappearance", 
...
Maternal effect senescence was the most likely explanation for declining offspring survival with maternal age, although the evidence for selective appearance and disappearance was stronger for offspring survival than for maternal reproduction. The AIC weights for the base, selective appearance, selective disappearance, and combined models were 0.421, 0.254, 0.206, and 0.119, respectively, which indicates selective appearance and disappearance cannot be disregarded. However, as with maternal reproduction, the coefficients for ages at first (95% CI: -0.143 - 0.047) and last (95% CI: -0.263 - 1.09) reproduction were not significantly greater than 0 and the selective appearance/disappearance models did not generate qualitatively different predictions than the senescence-only model [@fig-selective-h2].
