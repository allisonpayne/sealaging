...
all_years <- sealdat %>% 
  group_by(animalID, yearborn, longevity) %>% 
  summarize(age4 = yearborn[1] + 4,
            first_obs = min(year),
            last_obs = max(year),
            .groups = "keep") %>% 
  reframe(year = seq(age4, last_obs)) %>% 
  filter(between(year, 1987, 2020)) %>% 
  left_join(select(sealdat, animalID, year, observed),
            by = c("animalID", "year")) %>% 
  mutate(observed2 = factor(ifelse(is.na(observed), 
                                   "Non-breeder", 
                                   observed),
                            levels = c("Non-breeder", "Breeder")),
         year_fct = factor(year),
         age = year - yearborn,
         age10 = age / 10)
...
First, address detection probability. We do resights near-daily during breeding and molting seasons, so while we may miss an individual on a given day, it is unlikely that we miss a marked individual throughout the entire breeding or molting season.

```{r}
#| label: detection-prob

full <- semi_join(resight, sealdat, "animalID") %>% 
  mutate(fiscalyear = ifelse(yday > 200, calyear + 1, calyear)) %>% 
  group_by(animalID, fiscalyear) %>% 
  count(fiscalyear) %>% 
  mutate(year = as.numeric(fiscalyear)) %>% 
  filter(year > 1987)

full %>% 
  group_by(year) %>% 
  summarize(across(n, list(median = median, 
                           q25 = \(x) quantile(x, 0.25), 
                           q75 = \(x) quantile(x, 0.75)))) %>% 
  ggplot(aes(year, n_median)) +
  geom_ribbon(aes(ymin = n_q25, ymax = n_q75), fill = "grey80") +
  geom_line() +
  geom_hline(yintercept = 3, linetype = "dashed", color = "firebrick") +
  labs(y = "Resights per year") + 
  theme_classic()

#some animals have multiple rows because they have multiple pups associated with them...
sealdat %>% 
...
skips <- sealdat %>% 
  ...
  group_by(animalID) %>% 
  summarize(n_obs = n(),
            obs_window = max(year) - min(year) + 1,
            skips = obs_window - n_obs) %>% 
  group_by(obs_window, skips) %>% 
  summarize(ind = n(), .groups = "drop")

resights_per_year <- full %>% 
  group_by(year) %>% 
  summarize(median_resights = median(n))

skips_per_year <- skips

skip_frac <- sealdat %>% 
  distinct(animalID, year) %>% 
  group_by(animalID) %>% 
  summarize(firstobs = min(year),
            lastobs = max(year)) %>% 
  mutate(n = lastobs - firstobs + 1) %>% 
  uncount(n) %>% 
  group_by(animalID) %>% 
  mutate(year = firstobs + seq(n()) - 1) %>% 
  ungroup() %>% 
  left_join(transmute(sealdat, animalID, year, skip = 0), 
            by = c("animalID", "year")) %>% 
  mutate(skip = ifelse(is.na(skip), 1, 0)) %>% 
  group_by(year) %>% 
  summarize(skip_frac = sum(skip) / n())

resights_per_year %>% 
  left_join(skip_frac, by = "year") %>% 
  ggplot(aes(median_resights, skip_frac)) +
  geom_point(aes(fill = year), shape = 21, color = "grey20", size = 2) +
  ggrepel::geom_label_repel(aes(label = year), max.overlaps = 8) +
  scale_y_continuous("Skip fraction", labels = scales::percent) +
  scale_fill_distiller(palette = "PuRd") +
  labs(x = "Median resights") +
  theme_classic()

#mark recapture model is necessary? could have bred at another colony. Many skips!!

ggplot(skips, aes(obs_window, skips, size = ind)) +
  geom_point()
...
breed_age_mod <- glmer(
  observed2 ~ age10 + (1 | animalID) + (1 | year_fct),
  data = all_years, 
  family = "binomial",
...
year_ranef2 <- slice(year_ranef, 
                     round(seq(1, nrow(year_ranef), length.out = 5)))

indiv_ranef <- ranef(breed_age_mod)$animalID %>% 
  as_tibble(rownames = "animalID") %>% 
  mutate(animalID = factor(animalID)) %>% 
  rename(ranef = 2) %>% 
  arrange(ranef)

indiv_ranef2 <- slice(indiv_ranef, 
                      round(seq(1, nrow(indiv_ranef), length.out = 5)))

breed_age_pred_year <- ggpredict(breed_age_mod, 
                                 terms = c("age10 [all]", "year_fct"), 
                                 type = "random",
                                 interval = "confidence") %>% 
  as_tibble() %>% 
  mutate(age = x * 10,
  ...
  semi_join(year_ranef2, by = "year") %>% 
  filter(age <= 20) 

breed_age_pred_indiv <- ggpredict(breed_age_mod, 
                                  terms = c("age10 [all]", "animalID"), 
                                  type = "random",
                                  interval = "confidence") %>% 
  as_tibble() %>% 
  mutate(age = x * 10,
         animalID = group) %>% 
  semi_join(indiv_ranef2, by = "animalID") %>% 
  filter(age <= 20) 

breed_age_pred <- ggpredict(breed_age_mod, 
                                 terms = "age10 [all]", 
                                 type = "fixed",
                                 interval = "confidence") %>% 
  as_tibble() %>% 
...
breed_nonbreed <- all_years %>% 
  group_by(age) %>% 
  summarize(n_observed = sum(!is.na(observed2)), 
            perc_breed = sum(observed2 == "Breeder") / n_observed, 
            se = (perc_breed * (1 - perc_breed) / n_observed)^0.5) %>% 
  filter(age <= 20)

pred_year_plot <- ggplot(breed_age_pred_year, aes(age)) +
  ...
  # Predictions for specific years (random effect)
  ...
  geom_pointrange(aes(y = perc_breed, 
                  ...
                  breed_nonbreed) + 
  # Year labels
  geom_text(aes(y = predicted, label = year_fct), 
            filter(breed_age_pred_year, age == 20),
            x = 20.2,
            hjust = 0,
            size = 3) +
  scale_x_continuous("Age", limits = c(NA, 21)) +
  scale_y_continuous("Breeding %", 
                     limits = c(0, 1), 
                     labels = scales::percent) +
  theme_classic()

pred_indiv_plot <- ggplot(breed_age_pred_indiv, aes(age)) +
  # Prediction across years (fixed effect only)
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high),
              breed_age_pred,
              fill = "grey", alpha = 0.5) +
  geom_line(aes(y = predicted), 
            ...
            color = "firebrick", linewidth = 1.5) +
  # Predictions for specific years (random effect)
  geom_line(aes(y = predicted, group = animalID), 
            color = "cornflowerblue", linewidth = 1) +
  # Raw data
  geom_pointrange(aes(y = perc_breed, 
                      ymin = perc_breed - se, 
                  ...
                  breed_nonbreed) + 
  # Individual labels
  geom_text(aes(y = predicted, label = animalID), 
            filter(breed_age_pred_indiv, age == 20),
            x = 20.2,
            hjust = 0,
            size = 3) +
  scale_x_continuous("Age", limits = c(NA, 21)) +
...
pred_year_plot | pred_indiv_plot

year_plot <- all_years %>% 
  filter(age <= 20) %>% 
  left_join(year_ranef, by = "year") %>% 
  group_by(age) %>% 
  summarize(across(ranef, list(mean = mean, sd = sd))) %>% 
  ggplot(aes(age, ranef_mean)) +
  geom_pointrange(aes(ymin = ranef_mean - ranef_sd,
                      ymax = ranef_mean + ranef_sd)) +
  geom_hline(yintercept = 0, color = "firebrick", linewidth = 1.5) +
  labs(x = "Age", y = "Year ranef") +
  theme_classic()

indiv_plot <- all_years %>% 
  filter(age <= 20) %>% 
  left_join(indiv_ranef, by = "animalID") %>% 
  group_by(age) %>% 
  summarize(across(ranef, list(mean = mean, sd = sd))) %>% 
  ggplot(aes(age, ranef_mean)) +
  geom_pointrange(aes(ymin = ranef_mean - ranef_sd,
                      ymax = ranef_mean + ranef_sd)) +
  geom_hline(yintercept = 0, color = "firebrick", linewidth = 1.5) +
  labs(x = "Age", y = "Indiv ranef") +
  theme_classic()

year_plot | indiv_plot
  
```

```{r}
#| label: age_breeding_b4after

senescence_age <- 12

breeding_mod_b4 <- glmer(
  observed2 ~ age10 + (1 | animalID) + (1 | year_fct),
  data = filter(all_years, age <= senescence_age), 
  family = "binomial",
  control = glmerControl(optimizer="bobyqa")
)

breeding_mod_after <- glmer(
  observed2 ~ age10 + (1 | animalID) + (1 | year_fct),
  data = filter(all_years, age > senescence_age), 
  family = "binomial",
  control = glmerControl(optimizer="bobyqa")
...
summary(breeding_mod_b4)
summary(breeding_mod_after)

```

```{r}
#| label: fig-age_breeding
...
breed_pred <- rbind(
  ggpredict(breeding_mod_b4,
            terms = "age10 [all]") %>% 
    as_tibble(),
  ggpredict(breeding_mod_after,
            terms = "age10 [all]") %>% 
    as_tibble()
) %>% 
    mutate(age = x * 10,
           age_class = ifelse(age <= senescence_age,
                              "Before",
                              "After"))
...
breed_nonbreed <- all_years %>% 
  ...
  geom_ribbon(aes(age, ymin = conf.low, ymax = conf.high, group = age_class),
              breed_pred,
              inherit.aes = FALSE,
              fill = "#829BD1",
              alpha = 0.25) +
  geom_line(aes(age, predicted, group = age_class),
            breed_pred,
            inherit.aes = FALSE,
            color = "#829BD1",
            linewidth = 1) +
  geom_point() + 
  geom_errorbar(aes(ymin = perc_breed - se, ymax = perc_breed + se), 
                width = 0) +
...
ranef_year <- ranef(breeding_mod)$year_fct %>% 
  as_tibble(rownames = "year_fct") %>% 
  arrange(`(Intercept)`) %>% 
  slice(1, floor(nrow(.) * c(0.25, 0.5, 0.75, 1))) %>% 
  expand_grid(age10 = unique(sealdat$age10)) %>% 
  mutate(year_fct = factor(year_fct, levels = levels(sealdat$year_fct))) %>% 
  mutate(predicted = predict(breeding_mod, 
                             newdata = ., 
                             re.form = ~ (1 | year_fct), type = "response"),
         age = age10 * 10)
...
breed_nonbreed <- sealdat %>% 
  ...
  geom_ribbon(aes(age, ymin = conf.low, ymax = conf.high),
              ...
              fill = "#829BD1",
              alpha = 0.25) +
  geom_line(aes(age, predicted, group = year_fct),
            ranef_year,
            inherit.aes = FALSE,
            alpha = 0.2) +
  geom_text(aes(x = 22.1, predicted, label = year_fct),
