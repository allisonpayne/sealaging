...
breed_age_mod1 <- glmer(
    observed_int ~ age10 + (1 | animalID) + (1 | year_fct),
    data = filter(sealdat, age < age_senesce),
    family = "binomial",
...
get_ranefs <- function(model, r, reduce = TRUE) {
  result <- ranef(model)[[r]] %>% 
    as_tibble(rownames = r) %>% 
    rename(ranef = 2) %>% 
    arrange(ranef)
  
  if (reduce)
    slice(result, round(seq(1, nrow(result), length.out = 5)))
  else 
    result
}

year_ranef1 <- get_ranefs(breed_age_mod1, "year_fct")
year_ranef2 <- get_ranefs(breed_age_mod2, "year_fct")
indiv_ranef1 <- get_ranefs(breed_age_mod1, "animalID")
indiv_ranef2 <- get_ranefs(breed_age_mod2, "animalID")

get_ranef_pred <- function(model, r, keep) {
  ggpredict(model,
            terms = c("age10 [all]", r), 
                             type = "random",
                             interval = "confidence") %>% 
...
breed_pred_year <- rbind(
  get_ranef_pred(breed_age_mod1, "year_fct", year_ranef1) %>% 
    mutate(age_class = "young"),
  get_ranef_pred(breed_age_mod2, "year_fct", year_ranef2) %>% 
    mutate(age_class = "old")
) %>% 
  filter(age <= 20)

breed_pred_indiv <- rbind(
  get_ranef_pred(breed_age_mod1, "animalID", indiv_ranef1) %>% 
    mutate(age_class = "young"),
  get_ranef_pred(breed_age_mod2, "animalID", indiv_ranef2) %>% 
    mutate(age_class = "old")
) %>% 
  filter(age <= 20)
...
ggplot(breed_pred_year, aes(age)) +
  ...
  geom_line(aes(y = predicted, group = interaction(year_fct, age_class)),
  ...
  geom_text(aes(y = predicted, label = year_fct),
            filter(breed_pred_year, age == 20),
            x = 20.2,
            hjust = 0,
            size = 3) +
  geom_text(aes(y = predicted, label = year_fct),
            filter(breed_pred_year, age == 4),
            x = 3.8,
            hjust = 1,
            size = 3) +
  scale_x_continuous("Age", limits = c(3.5, 20.5)) +
