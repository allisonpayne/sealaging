...
breed_mod_summary$coefficients %>% 
  ...
  mutate(Parameter = c("Intercept", "Age (linear)", "Age (quadratic)"),
         ...
         P_value = format_pval(`Pr(>|z|)`, digits = 2)) %>%
  select(-`Pr(>|z|)`) %>% 
  relocate(Parameter) %>% 
  knitr::kable()
```
...
surv_recruit_summary %>% 
  ...
  labs(x = "Maternal Age (Years)", y = "Pup Survival or Recruitment/n") +
  theme_few() + 
...
#average the averages - global average of age-specific averages 
```
