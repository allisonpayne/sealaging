...
Trying segmented analysis. Note: `segmented` does NOT support mixed effect GLMs!

```{r}
#| label: age_breeding_seg
...
breed_nseg <- lapply(
  list(15, c(9, 15)),
  \(.psi) {
    segmented(breeding_mod2, seg.Z = ~age, psi = .psi)
  }
)
AIC(breeding_mod2, breed_nseg[[1]], breed_nseg[[2]]) %>% 
  as_tibble() %>% 
  mutate(model = paste(1:3, "segments")) %>% 
  transmute(model, AIC, dAIC = AIC - min(AIC)) %>% 
  arrange(dAIC)

breeding_best <- breed_nseg[[1]]
breeding_pred <- tibble(age = seq(4, 22, length.out = 100)) %>% 
  predict(breeding_best, 
          newdata = ., 
          type = "response", 
          interval = "confidence") %>% 
  as_tibble() %>% 
  set_names(c("bred", "bred_lwr", "bred_upr")) %>% 
  mutate(age = seq(4, 22, length.out = 100))
...
ggplot(breeding_pred, aes(age)) +
  geom_ribbon(aes(ymin = bred_lwr, ymax = bred_upr),
              fill = "grey90") + 
  geom_line(aes(y = bred)) +
  geom_point(aes(y = perc_breed), breed_nonbreed) + 
