...
# Fit separate GLMMs to the data subsetted before and after onset of senescence
# We a priori chose age 12 for that (age_senesce)
# Pre-senescence model
breed_age_mod1 <- glmer(
    observed_int ~ age10 + (1 | animalID) + (1 | year_fct),
    # Here's the age filter
    data = filter(sealdat, age < age_senesce),
    family = "binomial",
    control = glmerControl(optimizer="bobyqa")
)
# Post-senescence model
...
breed_summ <- tibble(
  ...
  age_coef = c(fixef(breed_age_mod1)["age10"],
               fixef(breed_age_mod2)["age10"]),
  age_coef_se = c(summary(breed_age_mod1)$coef["age10", "Std. Error"],
                  summary(breed_age_mod2)$coef["age10", "Std. Error"]),
  `p(coef<0)` = c(pnorm(summary(breed_age_mod1)$coef["age10", "z value"]),
                  pnorm(summary(breed_age_mod2)$coef["age10", "z value"]))
) 
...
# Create POPULATION-LEVEL (i.e. without rand effects) predictions for both
# models and row bind them together
breed_pred <- rbind(
  # Pre-senescence
  ggpredict(
    breed_age_mod1,
...
# Now create rand effect predictions
# This utility function pulls rand effect *coefs* from a model
...
# Get year and individual random effeccts from pre- and post-senescence models
...
# This util fun pulls rand effect *predictions* from a model
...
# Predictions based on 5 different *years* (min, q1, median, q3, max ranef
# coef)
...
# As above, but 5 different *individuals*
...
breed_fig <- ggplot(breed_pred, aes(x = age)) +
...
(breed_fig | breed_coef_fig) + plot_layout(widths = c(3, 1))
...
repro_raw <- surv_recruit_data %>% 
  mutate(pupsex = factor(pupsex, labels = c("Female", "Male"))) %>% 
  group_by(age, pupsex) %>% 
  summarize(n_born = n(), 
            n_surv = sum(pup_survived_int),
            perc_surv = n_surv / n_born, 
            se_surv = (perc_surv * (1 - perc_surv) / n_born)^0.5,
            n_recr = sum(puprecruited),
            perc_recr = n_recr / n_born,
            se_recr = (perc_recr * (1 - perc_recr) / n_born)^0.5,
            .groups = "drop") 

ggplot(surv_pred, aes(x = age)) +
  ...
  geom_pointrange(aes(y = perc_recr,
                  ...
                  shape = 21,
                  filter(repro_raw, pupsex == "Female")) +
  # Add visual indicator for senescence
  geom_vline(xintercept = age_senesce - 0.5, linetype = "dashed") +
  scale_x_continuous("Age", limits = c(3.5, 18.5)) +
  scale_y_continuous("Pup survival/recruitment", 
                     limits = c(0, 0.65), 
                     labels = scales::percent) +
  scale_color_manual(values = c("firebrick", "cornflowerblue")) +
  facet_wrap(~pupsex) +
