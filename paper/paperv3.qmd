...
mean_yb_phen <- pheno_raw %>% 
  filter(age < age_senesce, phase == "breeddur") %>% 
  summarize(mean_yb = mean(dur_days), 
            sd_yb = sd(dur_days))
mean_ob_phen <- pheno_raw %>% 
  filter(age >= age_senesce, phase == "breeddur") %>% 
  summarize(mean_ob = mean(dur_days), 
            sd_ob = sd(dur_days))
mean_ym_phen <- pheno_raw %>% 
  filter(age < age_senesce, phase == "moltdur") %>% 
  summarize(mean_ym = mean(dur_days), 
            sd_ym = sd(dur_days))
mean_om_phen <- pheno_raw %>% 
  filter(age >= age_senesce, phase == "moltdur") %>% 
  summarize(mean_om = mean(dur_days), 
            sd_om = sd(dur_days))
```

Overall, the breeding haul out was `r mean_yb_phen$mean_yb` ± `r mean_yb_phen$sd_yb` days for animals younger than 11 years old and `r mean_ob_phen$mean_ob` ± `r mean_ob_phen$sd_ob` days for animals 11 years old and older. The molting haul out was `r mean_ym_phen$mean_ym` ± `r mean_ym_phen$sd_ym` days for animals younger than 11 years old and `r mean_om_phen$mean_om` ± `r mean_om_phen$sd_om` days for animals 11 years old and older.
...
This needs to be rethought now!
